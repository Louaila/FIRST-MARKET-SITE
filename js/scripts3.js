...
let idproductDetail = params.get("id");

// Définissez la fonction findProductById
const findProductById = (product) => product.id === idproductDetail;

const item = data.find(findProductById);

// Récupérer la propriété related de l'objet
let result = data.find(product => {
  return product.id === idproductDetail;
});

const relatedProductsIds = result.related;

relatedProductsIds.forEach(idOfRelatedProduct => {
  let relatedProduct = data.find(product => product.id === idOfRelatedProduct);

  let template = `
    <div class="col mb-5">
        <div class="card h-100">
            <!-- Product image-->
            <img class="card-img-top" src="images/${relatedProduct.image}" alt="${relatedProduct.label}" />
            <!-- Product details-->
            <div class="card-body p-4">
                <div class="text-center">
                    <!-- Product name-->
                    <h5 class="fw-bolder">${relatedProduct.label}</h5>
                    <!-- Product price-->
  ...
  document.getElementById("relatedProduct").innerHTML += template;
});

let productTemplate = `
  <div class="col-md-6">
      <img class="card-img-top mb-5 mb-md-0" src="images/${item.image}" alt="${item.label}" />
  </div>
  <div class="col-md-6">
      <div class="small mb-1">${item.id}</div>
      <h1 class="display-5 fw-bolder">${item.label}</h1>
      <div class="fs-5 mb-5">
          <span class="text-decoration-line-through">€${(item.price - (item.price * item.discount) / 100).toFixed(2)}</span>
...
document.getElementById("detailProduct").innerHTML = productTemplate;